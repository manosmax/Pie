...
Home Assistant supports MQTT Discovery. This means that sensors can announce themselves with a special configuration message or  directly from the Home Assistant UI instead of being manually configured in YAML files.
We published to `homeassistant/binary_sensor/pir01_motion/config`. The payload is:

```
{
  "name": "PIR Motion Sensor", #name that will appear in the Home Assistant UI
  "state_topic": "smartbin/bin-01/pir-01/motion", #The different topic where the device will actually send its status updates 
  "payload_on": "detected", 
  "payload_off": "clear", 
  "device_class": "motion",
  "unique_id": "pir_01_motion", # A unique string for this specific entity.
  "device": {
    "identifiers": ["pir-01"],
    "name": "PIR Sensor 01",
    "model": "HC-SR501",
